...
export default function EditProductForm({change, editProduct}) {
    const [productInfo, setProductInfo] = useState({
        productId: editProduct.productId,
        productName: editProduct.productName,
        scrumMasterName: editProduct.scrumMasterName,
        productOwnerName: editProduct.productOwnerName,
        numberOfDevs: editProduct.Developers.length,
        Developers: editProduct.Developers,
        startDate: editProduct.startDate.replaceAll("/", "-"),
    ...
    const handleSubmit = (e) => {
        ...
        if (submitEnabled && submitEnabled2){
            ...
            delete newProductInfo['productId'];

            console.log(JSON.stringify(newProductInfo));

            const submitData = async () => {
                try {
                    const response = await fetch(API_URL + "edit_product/" + editProduct.productId, {
                        method: "PUT",
                        headers: {
